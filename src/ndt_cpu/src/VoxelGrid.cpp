...
void VoxelGrid<PointSourceType>::scatterPointsToVoxelGrid()
{
	...
	for (int pid = 0; pid < (int)source_cloud_->points.size(); pid++) {
...
{
	if ((int)new_cloud->points.size() <= 0) {
		return;
	}
	...
	/* Update octree */
	std::vector<Eigen::Vector3i> new_voxel_id((int)new_cloud->points.size());

	for (int i = 0; (int)i < (int)new_cloud->points.size(); i++) {
...
void VoxelGrid<PointSourceType>::updateVoxelContent(typename pcl::PointCloud<PointSourceType>::Ptr new_cloud)
{
	...
	for (int i = 0; i < (int)new_cloud->points.size(); i++) {
		PointSourceType p = new_cloud->points[i];
		Eigen::Vector3d p3d(p.x, p.y, p.z);
